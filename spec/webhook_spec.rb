...
RSpec.describe TrueTrial::Webhook do
  ...
  describe "delivery.failed event" do
    let(:failure_payload) { '{"event":"delivery.failed","data":{"order_id":"01HTEST","external_order_id":"ORD-1"}}' }
    let(:failure_signature) { OpenSSL::HMAC.hexdigest("SHA256", secret, failure_payload) }

    it "parses a delivery.failed payload" do
      event = described_class.construct_event(failure_payload, failure_signature, secret)
      expect(event["event"]).to eq("delivery.failed")
    end
  end
